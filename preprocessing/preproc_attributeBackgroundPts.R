...
# lkpEnvVars database
dbLookup <- dbConnect(SQLite(), here("_data","databases","SDM_lookupAndTracking_terres.sqlite"))
...
# }
  ...
  names(tp) <- "fid"
  dbWriteTable(db, paste0(pts_table, "_att15"), sampsAtt, overwrite = TRUE, field.types = tp)
  ...
  for(i in 1:length(brkgrps)){
    ...
    # write to DB
    dbWriteTable(db, paste0(pts_table, "_att15"), sampsAtt, overwrite = FALSE, append = TRUE)
