...
exclude_these <- c("hymehenr", "linuwest", "phanwest", "rhodchap", "aescprat") #Delete codes in this vector when they are ready to be packaged
...
not_yet_exported<-c("pitumugi","sidethor")
...
for (j in 1:length(not_yet_exported)){
  ...
  for(algo in ensemble_algos){
    ...
    ## Not working for current round 8/22/2021 so commenting out and setting to 1 as I believe all are the first iteration
  ...
  print(paste0("staging_path: ",staging_path))
